...
{
  delete _meter;
  _meter = new MeshProgressBar(static_cast<float>(GetSize().x), static_cast<float>(GetSize().y), glm::vec3(1, 0, 0));
}


void WidgetWeaponStatus::Draw() const
{
  _meter->SetValue(static_cast<float>(std::clamp(_spaceship->GetWeaponHeat(0), 0.0, 1.0)));
